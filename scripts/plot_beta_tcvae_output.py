...
def load_losses(name, lattice_sites, interval, num_samples, scaled, seed, prfx, num_batches):
    ...
    vae_loss_history = list(losses[:, :, 0].reshape(-1))
    tc_loss_history = list(losses[:, :, 1].reshape(-1))
    rc_loss_history = list(losses[:, :, 2].reshape(-1))
    vae_loss = np.array(vae_loss_history).reshape(-1, num_batches)
    tc_loss = np.array(tc_loss_history).reshape(-1, num_batches)
    rc_loss = np.array(rc_loss_history).reshape(-1, num_batches)
...
def plot_diagram(data, temp_x, temp_y, cmap, file_prfx, alias):
    ...
    # label ticks
    ax.set_xticks(np.arange(temp_y.size), minor=True)
    ax.set_yticks(np.arange(temp_x.size), minor=True)
    ax.set_xticks(np.arange(temp_y.size)[::4], minor=False)
    ax.set_yticks(np.arange(temp_x.size)[::4], minor=False)
    ax.set_xticklabels(np.round(temp_y, 2)[::4], rotation=-60)
    ax.set_yticklabels(np.round(temp_x, 2)[::4])
    # label axes
    ax.set_xlabel(r'$T_y$')
    ax.set_ylabel(r'$T_x$')
    # place colorbal
...
if __name__ == '__main__':
    ...
    if np.any(np.array([ALPHA, BETA, LAMBDA]) > 0):
        ...
        PMU = load_output_data('vae_mean_p', NAME, N, I, NS, SC, SEED, PRFX)
        PSIGMA = load_output_data('vae_sigma_p', NAME, N, I, NS, SC, SEED, PRFX)
        PZ = load_output_data('vae_z_p', NAME, N, I, NS, SC, SEED, PRFX)
        plot_diagrams(MU, SIGMA, TX, TY, CM, NAME, N, I, NS, SC, SEED, PRFX, ['m', 's'], VERBOSE)
        plot_diagrams(PMU, PSIGMA, TX, TY, CM, NAME, N, I, NS, SC, SEED, PRFX, ['m_p', 's_p'], VERBOSE)
        plot_diagrams(Z, None, TX, TY, CM, NAME, N, I, NS, SC, SEED, PRFX, 'z', VERBOSE)
        plot_diagrams(PZ, None, TX, TY, CM, NAME, N, I, NS, SC, SEED, PRFX, 'z_p', VERBOSE)
    else:
        Z = load_output_data('vae_z', NAME, N, I, NS, SC, SEED, PRFX)
        PZ = load_output_data('vae_z_p', NAME, N, I, NS, SC, SEED, PRFX)
        plot_diagrams(Z, None, TX, TY, CM, NAME, N, I, NS, SC, SEED, PRFX, 'z', VERBOSE)
    ...
    L = load_losses(NAME, N, I, NS, SC, SEED, PRFX, BS)
